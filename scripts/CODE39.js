...
function enableGeneration() {
  ...
  showStatus(
    `Successfully loaded ${serialNumbers.length} records. Ready to generate barcodes!`,
    "success"
  );
}
...
function generateBarcodes() {
  ...
  const previewCount = Math.min(serialNumbers.length, 20);
  ...
  serialNumbers.slice(0, previewCount).forEach((serial, index) => {
    if (serial) {
      ...
      // Generate the CODE39 barcode
      bwipjs.toCanvas(canvas, {
        bcid: "code39", // Barcode type CODE39
  ...
  if (serialNumbers.length > previewCount) {
    ...
    moreInfo.innerHTML = `
          ...
          Showing ${previewCount} of ${serialNumbers.length} barcodes in preview. 
          All ${serialNumbers.length} barcodes will be included in the PDF.
        `;
...
["barcodeWidth", "barcodeHeight", "columns", "rows", "dynamicText"].forEach(
  (id) => {
...
function generatePDF() {
  ...
  processCanvases(0, [], (imageDataArray) => {
    ...
    // Calculate margins to center barcodes on the page
    ...
    imageDataArray.forEach((data, index) => {
      if (data.imgData) {
        ...
        // Add dynamic text below the barcode
        if (dynamicText) {
    ...
    const sanitizedText = dynamicText
      ...
      : "barcodes";
    const fileName = `code39_${sanitizedText}_${
