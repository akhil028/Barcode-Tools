...
function processData(data, fileName) {
  ...
  data.forEach((row) => {
    ...
    if (Array.isArray(row)) {
      ...
      value = keys.length > 0 ? row[keys[0]] : "";
    } else {
      value = row;
    }

    if (value && value.toString().trim() !== "") {
      productDetails.push(value.toString().trim());
    }
  });
  ...
  displayFileInfo(fileName, productDetails.length);
  generateBarcodes();
}
...
function displayFileInfo(fileName, count) {
  ...
  fileDetails.innerHTML = `
        <div class="row">
          ...
          <div class="col-sm-6"><strong>QR Codes to Generate:</strong> ${count.toLocaleString()}</div>
        </div>
      `;
...
function enableGeneration() {
  ...
  showStatus(
    `Successfully loaded ${productDetails.length.toLocaleString()} items. Ready to generate QR codes!`,
    "success"
  );
}
...
function generateBarcodes() {
  ...
  // Show first 12 QR codes for preview
  const previewCount = Math.min(productDetails.length, 12);
  ...
  productDetails.slice(0, previewCount).forEach((detail, index) => {
    ...
    textElement.title = detail; // Show full text on hover
    qrItem.appendChild(textElement);

    barcodesContainer.appendChild(qrItem);
  });
  ...
  if (productDetails.length > previewCount) {
    ...
    moreInfo.innerHTML = `
          ...
          Showing ${previewCount} of ${productDetails.length.toLocaleString()} QR codes in preview. 
          All ${productDetails.length.toLocaleString()} QR codes will be included in the PDF.
        `;
    barcodesContainer.appendChild(moreInfo);
  }
}

// Add event listeners to regenerate QR codes when settings change
  ...
  const processCanvases = (index, imageDataArray, callback) => {
    ...
    if (canvasElement instanceof HTMLCanvasElement) {
      canvasToImageData(canvasElement, (imgData) => {
        imageDataArray.push({ imgData, label: productDetails[index] });
        processCanvases(index + 1, imageDataArray, callback);
      });
    } else {
      imageDataArray.push({ imgData: null, label: "" });
      processCanvases(index + 1, imageDataArray, callback);
    }
  };
